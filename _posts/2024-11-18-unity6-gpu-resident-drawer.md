---
title: 'Unity6 GPU Resident Drawer 源码解析'
date: 2024-12-04
permalink: /posts/2024/1204/GPU-Resident-Drawer/
tags:
  - Unity
  - GPU Resident Drawer
  - GPU Driven
...
- 必须开启Forward+渲染。
- 平台必须支持Compute Shader，不支持OpenGL ES。
- 仅Mesh Renderer组件的GameObject。

其实还有其他的一些限制Unity没有详细说明，比如：
- Shader必须支持DOTS Instancing。
- 必须开启RenderGraph。
- 使用GPU Resident Drawer时，LightMap实现变成TextureArray索引方式获取。
- 
...
![Frame Debugger Hybrid Batch Group截图](../images/FrameDebugger.jpeg "Frame Debugger Hybrid Batch Group截图")

GPU Resident Drawer较传统SRP Batcher进行了优化:
- 在传统SRP Batcher中，SRP Batch是根据Mesh Renderer的顺序进行合批的，Renderer之间会因为数据不同而打断合批（比如材质、Keywords不同等），而GPU Resident Drawer将场景中的Renderer通过remapping合为更少的Batch，能获得更少的批次。
- 在传统SRP Batcher中，上传到GPU Memory的Buffer是per object的large buffer，是AOS的组织方式，而GPU Resident Drawer是DOTS的SOA的组织方式，将相同的数据连续存储，也就能得到更高的性能。
- 在传统SRP Batcher中组织Batch只在主线程中单线程执行，性能较GPU Resident Drawer差很多。

![Batch](../images/Batch.jpeg "Batch")
![SOA](../images/SOA.jpeg "SOA")
![AOS](../images/AOS.jpeg "AOS")

## CPU端剔除
首先我们来分析GPU Resident Drawer中剔除部分的源码，GPU Resident Drawer会每帧根据摄像机、光源及遮挡关系来对渲染物体做剔除，保证尽可能只渲染看得到的renderer。

首先是CPU端的摄像机视椎体剔除和光源阴影的剔除，对于渲染相机和产生阴影的光源，GPU Resident Drawer中会对其每个裁剪平面进行组织，每四个Plane会打成一个Pack，在源码的FrustumPlanes脚本下可以找到，下面是其部分核心代码：
```c#
...
```
![Packed4](../images/Packed4.jpeg "Packed4")
...
![TwoPhaseOcclusionCulling](../images/TwoPhaseOcclusionCulling.jpeg "TwoPhaseOcclusionCulling")
...
![OccluderDepthPyramid](../images/OccluderDepthPyramid.jpeg "OccluderDepthPyramid")
...
## 整体流程梳理
- 通过上文我们了解了完整的instance剔除流程，接下来我们来梳理完整的GPUResidentDrawer执行流程。
- GPUResidentDrawer在构造时通过InsertIntoPlayerLoop函数将自己的PostPostLateUpdate注册进了PlayerLoop的FinishFrameRendering上，进而即可在FinishFrameRendering时执行处理Material、Mesh和LODGroup相关的数据。
- 同时GPUResidentDrawer内部声明了GPUResidentBatcher，可以通过GPUResidentBatcher来创建写入及更新instance的相关数据。
- GPUResidentBatcher也可以通过调用InstanceCullingBatcher来执行遮挡剔除和视椎体剔除，GPUResidentBatcher每帧去更新Renderer的相关数据，并将其打成Batch。
- InstanceCullingBatcher内部维护着BatchRendererGroup，于是便可以通过BRG来注册删除Mesh、Material及Batch，并且通过BRG的OnPerformCulling回调去调用InstanceCuller的CreateCullJobTree来进行视椎体剔除、Binning、PrefixSum等操作，InstanceCuller内部同样封装着InstanceOcclusionTest来进行GPU的遮挡剔除操作。
- 通过BRG来执行DrawCommandOutputPerBatch，进而利用到了新增的BatchDrawCommandIndirect，最后整套流程得以走通，完成了整体的数据准备，便可以开始绘制。
...
这些东西感兴趣的读者可以自行查看源码来深入了解，本文章就不做更全面的分析了。总体来看Unity的新GPU Driven管线还是没能脱离原本的BRG和DOTS，没能做到更细粒度（Meshlet）的剔除和渲染，也就更做不到Nanite的无极LOD等等技术了。但是BRG新增的BatchDrawCommandIndirect和BatchDrawCommandProcedural可以让开发者自身做出更细致更定制化的技术方案，也算是个不错的选择。写到这里这篇GPU Resident Drawer的源码解析也就结束了，本人技术水平尚低，如果有大佬发现了文章中的错误，希望可以联系我去指正修改~

## 参考资料
- 1.解密GPU Resident Drawer——2023Unity技术开放日广州站·李中元 https://www.bilibili.com/video/BV1Q64y1H7oW/?vd_source=f6aefc124168040c3f678a9556622197
- 2.GPU Driven Rendering In Unity https://discussions.unity.com/t/gpu-driven-rendering-in-unity/930675
- 3.Unity官方文档 https://docs.unity3d.com/Manual/batch-renderer-group.html
- 3.知乎@清盐 https://zhuanlan.zhihu.com/p/831482785